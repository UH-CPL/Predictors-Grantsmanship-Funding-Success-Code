...
Data<-read.csv(file.path(curated_data_dir,data_file_name),stringsAsFactors = FALSE)
Raw_Data<-read.csv(file.path(curated_data_dir,data_file_name_std),stringsAsFactors = FALSE)
colnames(Data)<- c("Rank", "Mental", "Physical", "Temporal", "Performance", "Effort", "Frustration", "NASA","TA", "EXT", "AGR", "CS", "NT", "OP", "AV", "EM", "Task")
```


```{r, echo=FALSE}
Corelation_DF<-data.frame(Data[,c(1,8:17)], Raw_Data[,c(6,15:17,22,26,29:33,43,44)])
Corelation_DF<-Corelation_DF  %>%  dplyr::rename(H="google_scholar_h_index",
                                           ...
                                           SR="hy",                           ## Success Rate in the proposal
                                           AP="how_far_in_advance_do_you",# Composing Proposal
                                           AR="what_is_the_length_of_supp",   ## Length of supporting research
                                           DWH="in_the_week_leading_to_a_p",   ## Work load a week before Proposal Deadline
                                           DWR="week_research",            ## Research percentage a week before Proposal
                                           # Research_DD="proposal_research",        ## Research percentage in the Proposal deadlines day
                                           T="when_you_typically_subm",     ## Submit Proposal
...
for(i in 1:nrow(Corelation_DF)){
  ...
  if (Corelation_DF$WH[i]=="< 30"){
    ...
    Corelation_DF$WH[i]=3
  } else {
    Corelation_DF$WH[i]=4
  }
  ...
  if (Corelation_DF$BR[i]=="Every hour"){
    ...
    Corelation_DF$BR[i]=3
  } else {
    Corelation_DF$BR[i]=4
  }
...
cols.num <- c("RS", "WH", "BR", "NP", "SR", "AP", "AR", "DWH", "T", "DS")
Corelation_DF[cols.num] <- sapply(Corelation_DF[cols.num],as.numeric)

table(Corelation_DF$NP)
```


```{r, echo=FALSE}
file_name='CorrelationData.csv'
write.csv(Corelation_DF,file.path(curated_data_dir, file_name), row.names = FALSE)
```
...
M_proposal <- cor(Corelation_DF[,c(2:24)],use="complete.obs")
res1 <- cor.mtest(Corelation_DF[,c(2:24)], conf.level = .95)

# significance_plot<-
  corrplot(M_proposal, p.mat = res1$p, method = "color", type = "upper",
           title="Psychometrics-core questions correlation on proposals",
         sig.level = c(.001, .01, .05), pch.cex = .9,diag = FALSE,
         insig = "label_sig", pch.col = "black", col=colorRampPalette(c("brown1", "white", "dodgerblue"))(200), tl.col="black", mar=c(0,0,1,0))
```
...
for (i in 1:nrow(Corelation_DF_Conf)){
   if (Corelation_DF_Conf$NCP[i]=="1-2"){
    ...
    Corelation_DF_Conf$NCP[i]=4
  } else {
    Corelation_DF_Conf$NCP[i]=5
  }
  ...
  if (Corelation_DF_Conf$CR[i]=="C"){
    ...
    Corelation_DF_Conf$CR[i]=4
  }
  
  if (Corelation_DF_Conf$SR[i]=="10-20%"){
    Corelation_DF_Conf$SR[i]=1
  } else if (Corelation_DF_Conf$SR[i]=="20-30%"){
    Corelation_DF_Conf$SR[i]=2
  } else if (Corelation_DF_Conf$SR[i]=="30-50%"){
    Corelation_DF_Conf$SR[i]=3
  } else if (Corelation_DF_Conf$SR[i]=="50-75%"){
    Corelation_DF_Conf$SR[i]=4
  } else if (Corelation_DF_Conf$SR[i]=="75-90%"){
    Corelation_DF_Conf$SR[i]=5
  } else{
    Corelation_DF_Conf$SR[i]=6
  }
  ...
  if (Corelation_DF_Conf$ACP[i]=="< 1 week"){
    ...
    Corelation_DF_Conf$ACP[i]=4
  } else {
    Corelation_DF_Conf$ACP[i]=5
  }
  ...
  if (Corelation_DF_Conf$DS[i]=="Extremely less"){
    ...
    Corelation_DF_Conf$DS[i]=4
  } else if (Corelation_DF_Conf$DS[i]=="Extremely  more"){
    Corelation_DF_Conf$DS[i]=5
  } else {
...
res2 <- cor.mtest(Corelation_DF_Conf[,c(2:26)], conf.level = .95)
  ...
  corrplot(M_Conf, p.mat = res2$p, method = "color", type = "upper",
...
```{r}
Conf_C_Eng<- subset(Conf_C_Eng, departmental_affiliation=="Computer Science" | departmental_affiliation=="Engineering")
...
# par(mfrow = c(2, 2))
  corrplot(M_Conf_c_eng, p.mat = res3$p, method = "color", type = "upper",
...
# pdf(file = "corplot.pdf")
  corrplot(M_Conf_A_Astar, p.mat = res4$p, method = "color", type = "upper",
...
# significance_plot<-
# pdf(file = "corplot.pdf")
  corrplot(M_Conf_B_C, p.mat = res5$p, method = "color", type = "upper",

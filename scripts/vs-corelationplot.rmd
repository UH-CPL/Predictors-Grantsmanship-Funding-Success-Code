...
```{r, echo=FALSE}
Corelation_DF <-
  data.frame(Data[, c(1, 8:17)], Raw_Data[, c(6, 15:17, 22, 26, 29:33, 43, 44)])
Corelation_DF <-
...
cols.num <-
  c("RS", "WH", "BR", "NP", "SR", "AP", "AR", "DWH", "T", "DS")
...
# lbl$fit(Corelation_DF$Rank)
# Corelation_DF$Rank <- lbl$fit_transform(Corelation_DF$Rank)

rank_factor <- factor(Corelation_DF$Rank)
Corelation_DF$Rank <- as.numeric(rank_factor)

# Corelation_DF$BR<-factor(Corelation_DF$BR)
M_proposal <- cor(Corelation_DF[, c(1:24)], use = "complete.obs")
res1 <- cor.mtest(Corelation_DF[, c(1:24)], conf.level = .95)

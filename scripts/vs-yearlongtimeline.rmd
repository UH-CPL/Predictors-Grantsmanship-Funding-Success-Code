...
Timeline <- Timeline %>%
  mutate(NP_min = recode(NP, "1-2" = 1, "3-4" = 3,"5-6" = 5, "7-9" = 7, ">=10" = 10)) %>% 
  mutate(NP_mean = recode(NP, "1-2" = 1.5, "3-4" = 3.5,"5-6" = 5.5, "7-9" = 8, ">=10" = 10)) %>% 
  mutate(NP_max = recode(NP, "1-2" = 2, "3-4" = 4,"5-6" = 6, "7-9" = 9, ">=10" = 10)) %>% 
  
  mutate(NC_min = recode(NC, "1-2" = 1, "3-4" = 3,"5-6" = 5, "7-9" = 7, ">= 10" = 10)) %>% 
  mutate(NC_mean = recode(NC, "1-2" = 1.5, "3-4" = 3.5,"5-6" = 5.5, "7-9" = 8, ">= 10" = 10)) %>% 
  mutate(NC_max = recode(NC, "1-2" = 2, "3-4" = 4,"5-6" = 6, "7-9" = 9, ">= 10" = 10)) %>% 
  
  
  mutate(PCP_weeks_min = recode(PCP_weeks, "< 1 week" = 1, "1-2 weeks" = 1,"2-4 weeks" = 2, "1-2 months" = 4, "> 2 months" = 8)) %>% 
  mutate(PCP_weeks_mean = recode(PCP_weeks, "< 1 week" = 1, "1-2 weeks" = 1.5,"2-4 weeks" = 3, "1-2 months" = 6, "> 2 months" = 9)) %>% 
  mutate(PCP_weeks_max = recode(PCP_weeks, "< 1 week" = 1, "1-2 weeks" = 2,"2-4 weeks" = 4, "1-2 months" = 8, "> 2 months" = 10)) %>% 
  
  
  mutate(CCP_weeks_min = recode(CCP_weeks, "< 1 week" = 1, "1-2 weeks" = 1,"2-4 weeks" = 2, "1-2 months" = 4, "> 2 months" = 8)) %>% 
  mutate(CCP_weeks_mean = recode(CCP_weeks, "< 1 week" = 1, "1-2 weeks" = 1.5,"2-4 weeks" = 3, "1-2 months" = 6, "> 2 months" = 9)) %>% 
  mutate(CCP_weeks_max = recode(CCP_weeks, "< 1 week" = 1, "1-2 weeks" = 2,"2-4 weeks" = 4, "1-2 months" = 8, "> 2 months" = 10))
...
Timeline$Time_Min <- (Timeline$NP_min * Timeline$PCP_weeks_min) + (Timeline$NC_min * Timeline$CCP_weeks_min)
Timeline$Time_Mean <- (Timeline$NP_mean * Timeline$PCP_weeks_mean) + (Timeline$NC_mean * Timeline$CCP_weeks_mean)
Timeline$Time_Max <- (Timeline$NP_max * Timeline$PCP_weeks_max) + (Timeline$NC_max * Timeline$CCP_weeks_max)
...
Hist_min <- ggplot(Timeline, aes(x=Time_Min))+
  geom_histogram(color="darkblue", fill="lightblue")+
  geom_vline(aes(xintercept=mean(Time_Min)),
            color="blue", linetype="dashed", size=1)+ labs(x="", title = "Considering minimum time") + theme(plot.title = element_text(hjust = 0.5))

Hist_mean <- ggplot(Timeline, aes(x=Time_Mean))+
  geom_histogram(color="darkblue", fill="lightblue")+
    geom_vline(aes(xintercept=mean(Time_Mean)),
            color="blue", linetype="dashed", size=1) + labs(x="", title = "Considering mean time") + theme(plot.title = element_text(hjust = 0.5))

Hist_max <- ggplot(Timeline, aes(x=Time_Max))+
  geom_histogram(color="darkblue", fill="lightblue")+
      geom_vline(aes(xintercept=mean(Time_Max)),
            color="blue", linetype="dashed", size=1) + labs(x="[In Weeks]",title = "Considering maximum time") + theme(plot.title = element_text(hjust = 0.5))


final_plot <- ggarrange(Hist_min,Hist_mean, Hist_max,
                      nrow = 3,
                      ncol = 1)


  filename <- "Timeline_histogram.pdf"
  full_path <- file.path(plot_dir, filename)
  ggsave(
    full_path,
    final_plot,

...
BEmodel_75 <- glm(formula = FC ~ OP + H + RS + TWR + FA + TS, family = "binomial", data = lm_DF)
summary(BEmodel_75)
...
tmpd<-data.frame(FA,RS,TS,OP,H,TWR)
...
BEmodel_FF <- glm(FC ~ OP + TWR + FA, family = "binomial", data = lm_DF)
summary(BEmodel_FF)
```
...
BEmodel_FFSR50 <- glm(Y5Zff ~ E + AGR + TWR + NP, family = "binomial", data = lm_DF)
summary(BEmodel_FFSR50)
```
...
sectionA_plots <-
  function(Data,
    ...
    if (item == "FA" | item == "H" | item == "NP") {
      y_title = bquote( ~ italic("P"[.(paste0(suff, configuration))]))
...
configuration = "75"
for (item in 1:length(plot_item)){
  plot = sectionA_plots(Model_Data, plot_item[item], modified_ticx[[item]], title[item], configuration, color_list[[item]], "F")
  plot_list[[length(plot_list) + 1]] <- plot
}
...
# 
# plot5 = continous_features_plots(BEmodel_75, "H", H_actual, "h-index", 75)
...
plot5 = continous_features_plots(BEmodel_75, "H", H_actual, "h-index", 75, "F", blue)
RS_color = c("gray", blue)
...
plot7 <- SectionB_draw_dummy_plot("", "", "FF", "")
...
plot2 = continous_features_plots(BEmodel_75, "OP", H_actual, "Openness", "75", "F", blue)
plot3 = continous_features_plots(BEmodel_FF, "OP", H_actual, "", "", "", blue)
...
filename <- "Figure_FC_prediction_verision4.pdf"
...
plot4 = continous_features_plots(BEmodel_FFSR50, "AGR", H_actual, "Agreeableness", "S50FF",  "", blue)
plot4
...
filename <- "Figure_FCS50_prediction_verision4.pdf"
full_path <- file.path(plot_dir, filename)
ggsave(
  full_path,
  final_plot,
...
```{r, echo=FALSE}
plt = plot_model(BEmodel_FFSR50,
                 type = "pred",
                 terms = "TWR") +
  geom_line(aes(color="TWR"), size =1)+
  geom_smooth( method = "pred")+

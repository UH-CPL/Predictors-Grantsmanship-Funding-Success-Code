...
```{r, echo=FALSE}
dummy_df <- data.frame(x = c(0, 1, 0, 1), 
                       y = c(1, 0, 0, 1), 
                       group=c('a', 'a', 'b', 'b'))
...
SectionA_draw_dummy_plot <- function(plot_title, y_title) {
  if (plot_title!=""){
    title = element_text(hjust = 0.5, size = 8, face = "bold")
  }
  ...
  dummy_plot <- ggplot(dummy_df, aes(x = x, y = y, group = group)) +
    geom_line(scale = 0.9, color= "gray93") +
    # theme_minimal() +
    theme(
      plot.title = title,
...
SectionB_draw_dummy_plot <- function(plot_title, y_title) {
  dummy_plot <- ggplot(dummy_df, aes(x = x, y = y, group = group)) +
    geom_line(scale = 0.9, color= "gray93") +
    # theme_minimal() +
    theme(
...
```{r, echo=FALSE}
model_plot_sectionA <- function(model_name, item, modified, Plot_title, configuration) {
  print(model_name)
  ...
  if (configuration==20){
    plot_title_ = element_text(hjust = 0.5, face = "bold", size = 8)
  }
  ...
  if (item == "NP" |
    ...
    plt = plot_model(model_name, type = "pred", terms = item) +
      theme(
        ...
        axis.text.x = element_text(face = "bold", size = 7, angle = 30, hjust = 1),
        axis.text.y = element_text(face = "bold", size = 7)) +
...
model_plot <- function(model_name, item, modified, Plot_title) {
  ...
  if (item == "NP" |
    ...
    plt = plot_model(model_name, type = "pred", terms = item) +
      ...
      ) + theme(axis.text.y = element_blank(), axis.title.y = element_blank(), axis.ticks.y = element_blank())+ scale_x_discrete(limits = modified)
    return(plt)
  }
...
filename <- "SR_prediction_A_P.pdf"
...
p + geom_label(data=annotation, aes( x=x, y=y, label=label), 
           color="black", 
           size=5 , angle=180, fontface="bold" )
```

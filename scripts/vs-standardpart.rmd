...
```{r, echo=FALSE}
  ...
  bar_plot <- ggplot(data = temp, aes(x = item, y = cou)) +
    geom_bar(stat = "identity",
             width = 0.5,
             fill = "steelblue") +
    geom_text(aes(label=cou), vjust=1, color="white", size=3.5) +
    theme_bw() +
...
# H_index <- StandardPart[!is.na(StandardPart$faculty_rank),]

hindex_boxplot <-
  ggplot(StandardPart[!is.na(StandardPart$google_scholar_h_index),], 
         aes(
...
filename <- "WD_Proposal_Deadline_Week_Before.pdf"
full_path <- file.path(plot_dir, filename)
ggsave(
  full_path,
  work_distribution_before_PD,
...
filename <- "WD_Proposal_Deadline_Day.pdf"
full_path <- file.path(plot_dir, filename)
ggsave(
  full_path,
  proposal_day_work_distribution,
...
filename <- "WD_Conferance_Deadline_week_before.pdf"
full_path <- file.path(plot_dir, filename)
ggsave(
  full_path,
  conf_day_work_dist_b4_week,
...
filename <- "WD_Conferance_Deadline_Day.pdf"
full_path <- file.path(plot_dir, filename)
ggsave(
  full_path,
  paper_work_distribution,
...
filename <- "Proposal_Disruption.pdf"
full_path <- file.path(plot_dir, filename)
ggsave(
  full_path,
...
```{r, echo=FALSE}
library(freqtables)
data_file_name <- 'Selected_Core_Questions.csv'
curated_data_dir <- file.path(project_directory, 'curated-data')
Data_CQ <-
  read.csv(file.path(data_dir, data_file_name), stringsAsFactors = FALSE)

list1 = c("1-2", "3-4", "5-6", "7-9", ">=10")
list2 = c("1-2 weeks", "2-4 weeks", "1-2 months", "> 2 months")
list3 = c("< 30", "30-40", "40-50", "> 50")
list4 = c("Every hour", "Every 1-2 hours", "Every 3-4 hours", "Other")
list5 = c("Significantly less", "Less", "About the same", "More", "Significantly more")
list6 = c("Significantly less", "Less", "About the same", "More", "Significantly more")
...
title_list = c(
  ...
  "Typical week workload [in hrs]",
  "Walking breaks",
  "Proposal vs typical week workload",
  "Proposal deadline day vs regular working days workload"
)
...
ylimit = 250
for (i in 1:length(Data_CQ)) {
  if ((i %% 2) ==0) {
  temp <- count(Data_CQ[i])
  colnames(temp) <- c("item", "count")
  temp <- temp[!(temp$item == ""), ]
  temp <- temp[order(temp$count),]
  temp <- temp[complete.cases(temp), ]
  
  # temp2 = Data_CQ %>% freq_table(Data_CQ[3])
  # temp2 = temp2[, c(2, 8, 9)]
  ...
  # temp_final <- merge(temp, temp2, by="item")
  ...
  plot_list_grid[[length(plot_list_grid) + 1]] <- bar_plot
  }
  else{
  temp <- count(Data_CQ[i])
  colnames(temp) <- c("item", "count")
  ...
  # temp2 = Data_CQ %>% freq_table(Data_CQ[i])
  # temp2 = temp2[, c(2, 8, 9)]
  # temp2 <- temp2[!(temp2$cat == ""), ]
  # temp2 <- temp2[complete.cases(temp2), ]
  # temp2<-temp2  %>%  dplyr::rename( item="cat", LCI="lcl", UCI="ucl") 
  ...
  bar_plot <- ggplot(data = temp, aes(x = item, y = count)) +
    ...
    scale_y_continuous(breaks = seq(0, ylimit, by = 50),
                       limits = c(0, ylimit)) +
    labs(x = "", y = "Responses", title = title_list[i]) +
    theme(
...
stat = Trait_Anxiety %>% summarise(mean= round(mean(Sum), 2), sd = round(sd(Sum)), median = median(Sum), max = max(Sum),min = min(Sum))


x_lebel = paste0("\n\n\n", "Mean = ", stat$mean, ",     ", "SD = ", stat$sd, ",     ", "Median = ", stat$median, ",     ", "Min = ", stat$min, ",     ", "Max = ", stat$max)  

trit_boxplot <- ggplot(Trait_Anxiety, aes(x = "", y = Sum)) +
  geom_boxplot(fill='#A4A4A4', color="black") +
  theme_bw() +
  labs(x = x_lebel, y = "", title = "Trait Anxiety") +
  theme(
...
BFI_boxplots <-
  ggplot(bfi_score_df, aes(x = factor(Title), y = Value, fill = Title)) +
  geom_boxplot() +
  theme_bw() +
  ...
  theme(
    ...
    axis.text.y = element_text(size = 6, face = "bold")
                   ...
                   geom = "text",
                   size = 2) +
  scale_y_continuous(breaks = seq(1, 10, by = 1), limits = c(0, 10)) +
  guides(
    fill = guide_legend(
      nrow = 1,
      keywidth = 0.15,
...
# plot_list_grid[[length(plot_list_grid) + 1]] <- BFI_boxplots
...
colnames(CISS_Results)<-c("Record_ID", "Avoidance", "Emotion", "Task")

CiSS_Melt<-melt(CISS_Results[,c('Record_ID','Avoidance','Emotion','Task')],id.vars = 1)
...
# Stressful<-read.csv(file.path(curated_data_dir,data_file_name_std),stringsAsFactors = FALSE)
# 
...
# 
# set.seed(42)
# wc <- ggplot(Stressful, aes(label = item, size = count, color = factor(sample.int(10, nrow(Stressful), replace = TRUE)))) +
...
```{r, echo=FALSE}
# plot_list_grid[[length(plot_list_grid) + 1]] <- BFI_boxplots
# plot_list_grid[[length(plot_list_grid) + 1]] <- CISS_boxplots
# plot_list_grid[[length(plot_list_grid) + 1]] <- nasa_boxplots
# plot_list_grid[[length(plot_list_grid) + 1]] <- wc

barplots <- ggarrange(plotlist = plot_list_grid, nrow = 3, ncol = 2)
box_plots <- ggarrange(BFI_boxplots, nasa_boxplots, CISS_boxplots, nrow = 3, ncol = 1)

final_plot<-plot_grid(barplots, box_plots, ncol = 2,   rel_widths = c(2, 1))
# 
# final_plot <- ggarrange(plotlist = plot_list_grid, nrow = 3, ncol = 3)

full_path<-file.path(plot_dir,"Grid_plot_final.pdf",sep="")
  ggsave(full_path, final_plot, width = 8.5, height = 5.5, units = "in")
```

```{r}
# 
# plot_with_annotation <- function(plot_list, letter_list){
#   plot_list_grid_annot <- list()
#   
#   for (item in 1:10){
#     plot <- arrangeGrob(plot_list[item],
#               top = textGrob(
#                 toupper(paste0(letter_list[item])),
#                 x = unit(0, "npc"),
#                 y = unit(1, "npc"),
#                 vjust = 2,
#                 hjust = -1.5,
...
#     plot_grid(plot)
#     
#     plot_list_grid_annot[[length(plot_list_grid_annot) + 1]] <- plot_grid(plot)
#     
#   }
#   
#   return(plot_list_grid_annot)
# }
# 
# letter_list <-c("A", "B", "C", "D", "E", "F", "G", "H", "I", "J")
# 
# final_list =  plot_with_annotation(plot_list_grid, letter_list)
# 
# finla_plot = ggarrange(plotlist = final_list, nrow = 5, ncol = 2)
# 
# 
# 
# plot <-
...
Psychometric$Avoidance_Coping<-CISS_Results$Avoidance
Psychometric$Emotional_oriented_Coping<-CISS_Results$Emotion
Psychometric$Task_oriented_Coping<-CISS_Results$Task
...
Psychometric$h_index <- StandardPart$google_scholar_h_index
DF_Quant_Stat <- cbind(Psychometric, StandardPart[,c(17:20, 33:36, 38:41, 45:49, 58:61, 63:66, 70:74)])


DF_Quant_Stat <- DF_Quant_Stat  %>% dplyr::rename(
  
  "Percentage of research in a typical week" = "ti_research",  ## Typical week Research
  "Percentage of teaching in a typical week" = "ti_teaching",  ## Typical week Research
  "Percentage of admin work in a typical week" = "ti_administration",  ## Typical week Research
  "Percentage of service outreach in a typical week" = "ti_service_outreach",  ## Typical week Research
  
  "Percentage of research a week before proposal deadline" = "week_research",  ## Research percentage a week before Proposal
  "Percentage of teaching a week before proposal deadline" = "week_teaching",  ## Research percentage a week before Proposal
  "Percentage of admin work a week before proposal deadline" = "week_administration",  ## Research percentage a week before Proposal
  "Percentage of service outreach a week before proposal deadline" = "week_service_outreach",  ## Research percentage a week before Proposal

  "Percentage of research on proposal deadline day" = "proposal_research",
  "Percentage of teaching on proposal deadline day" = "proposal_teaching",
  "Percentage of admin work on proposal deadline day" = "proposal_administration",
  "Percentage of service outreach on proposal deadline day" = "proposal_service_outreach",
  
  "Disruption of research on proposal deadline" = "disrupted_part",
  "Disruption of sleep on proposal deadline" = "my_sleep",
  "Disruption of diet on proposal deadline" = "my_diet",
  "Disruption of physical activity on proposal deadline" = "my_physical_activity",
  "Disruption of interpersonal relationship on proposal deadline" = "my_interpersonal_relations",
  
  "Percentage of research a week before conference deadline" = "research",
  "Percentage of teaching a week before conference deadline" = "teaching",
  "Percentage of admin work a week before conference deadline" = "administration",
  "Percentage of service outreach a week before conference deadline" = "serviceoutreach",

  "Percentage of research on conference deadline day" = "research1",
  "Percentage of teaching on conference deadline day" = "teaching1",
  "Percentage of admin work on conference deadline day" = "administration1",
  "Percentage of service outreach on conference deadline day" = "service_outreach1",

  "Disruption of research on conference deadline" = "disrupted_conf",
  "Disruption of sleep on conference deadline" = "my_sleep2",
  "Disruption of diet on conference deadline" = "diet2",
  "Disruption of physical activity on conference deadline" = "physical_activity2",
  "Disruption of interpersonal relationship on conference deadline" = "interpersonal_relations2")
```
...
for (col_index in 2:ncol(DF_Quant_Stat)) {
  col = as.numeric(unlist(DF_Quant_Stat[col_index]))
  stat = DF_Quant_Stat %>% summarise(
    mean = round(mean(col, na.rm=TRUE), 2),
    sd = round(sd(col, na.rm=TRUE)),
  ...
  boxplot_with_stat <-
    ggplot(DF_Quant_Stat, aes(x = "", y = col)) +
    geom_boxplot(fill = '#A4A4A4', color = "black") +
    theme_bw() +
    labs(x = x_lebel,
         y = "",
         title = colnames(DF_Quant_Stat[col_index])) +
    theme(
  ...
  filename <- paste("boxplot_with_stat", col_index, ".pdf", sep = "")
  full_path <- file.path(plot_dir, filename)
  ggsave(
    full_path,
    boxplot_with_stat,
...
col = as.numeric(unlist(Psychometric[col_index]))
stat = Psychometric %>% summarise(mean= round(mean(col), 2), sd = round(sd(col)), median = median(col), max = max(col), min = min(col))
  ...
  boxplot_with_stat <-
    ggplot(Psychometric, aes(x = "", y = col)) +
    geom_boxplot(fill = '#A4A4A4', color = "black") +
    theme_bw() +
    labs(x = x_lebel,
         y = "",
         title = colnames(Psychometric[col_index])) +
    theme(
      axis.title.x = element_text(size = 16),
      panel.grid.major = element_blank(),
      panel.grid.minor = element_blank(),

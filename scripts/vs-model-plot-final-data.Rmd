...
data_processing_model <- function(data_file_name) {
  ...
  if (data_file_name == "ModelData_SR20.csv"){
      model_20 <- glm(SR ~ H + NP + FA + DS, family = "binomial", data = lm_DF)
      return(model_20)
  }
...
RS<-c(rep("1",9),"1","2", rep("1",8))
DS<-c(rep("1",11),"1","2", rep("1",6))
TS<-c(rep("1",13),"1","2", rep("1",4))
DWH<-c(rep("1",15),"1","2", rep("1",2))
H<-rep(29.56824,19)
TA<-rep(41.33747,19)
tmpd<-data.frame(NP,FA,RS,DS,TS,DWH,H,TA)
...
NP<-as.factor(c("1","2","3",rep("1",19)))
FA<-as.factor(c(rep("1",3),"1","2","3","4","5","6",rep("1",13)))
BF<-as.factor(c(rep("1",9),"1","2",rep("1",11)))
DS<-as.factor(c(rep("1",11),"1","2",rep("1",9)))
PR<-as.factor(c(rep("1",13),"1","2","3","4","5", rep("1",4)))
H<-rep(29.56824,22)
E<-rep(5.957816,22)
AV<-rep(18.81638,22)
Task<-rep(28.44665,22)
tmpd<-data.frame(NP,FA,BF,DS,PR,H,E,AV,Task)
...
# legend_plot = sectionA_plots(Model_Data, "NP", np_actual, "title[item]", "20", np_color)
# legend_plot
# legend_timeline<<-get_legend(legend_plot + add_bottom_legend())
# legend_timeline
```
...
PR_color = c("gray", blue, orange , blue, orange)
...
plot2 = continous_features_plots(model_30, "H", H_actual, "h-index", 30)
plot3 = continous_features_plots(model_50, "H", H_actual, "h-index", 50)
...
plot4 <- continous_features_plots(model_30, "TA", FR_actual, "Trait Anxiety", 30)
plot5 <- SectionA_draw_dummy_plot("", "")
plot6 <- SectionA_draw_dummy_plot("", "")

plot7 <- SectionA_draw_dummy_plot("", "")
plot8 = continous_features_plots(model_50, "E", H_actual, "", "")
plot9 = continous_features_plots(model_50, "AV", H_actual, "", "")
...
filename <- "SR_prediction_full_sample_ABC.pdf"
full_path <- file.path(plot_dir, filename)
ggsave(
  full_path,
  final_plot,

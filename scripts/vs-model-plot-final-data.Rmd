...
E<-rep(5.957816,21)
AV<-rep(18.81638,21)
tmpd<-data.frame(NP,FA,BF,DS,PR,H,E,AV)
...
Research_tactics <- plot_grid(Research_tactics, legend, nrow = 2, rel_heights = c(1, .03))
...
for (item in 1:length(plot_item)){
  ...
  plotDS30 = sectionA_plots(Model_Data, "DS", modified_ticx[[item]], "", 30, c("gray", orange))
}
...
final_plot <-
  plot_grid(Research_tactics,
            ...
            labels = c("a", "")) + annotation_custom(grid.polygon(
              c(0, 0.5, 1, 0.336, 0.336),
              c(0.5, 0.5, 0.5, 0, 0.5),
              id = c(1, 1, 1, 2, 2),
              gp = gpar(lwd = 1.5))) +  annotation_custom(grid.polygon(
              c(0, 0.25, 1, 1, 1),
              c(0.525, 0.525, 0.525, 0, 0.525),
              id = c(1, 1, 1, 2, 2),
              gp = gpar(lwd = 1.5)))  
...
filename <- "Figure_SR_prediction_version5.pdf"
full_path <- file.path(plot_dir, filename)
ggsave(
  full_path,
  final_plot,

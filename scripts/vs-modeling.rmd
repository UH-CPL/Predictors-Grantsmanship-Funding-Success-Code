...
# lm_DF <- lm_DF %>% mutate(value = 1)  %>% spread(Rank, value,  fill = 0 )
# lm_DF <- lm_DF %>% rename(Assistant_Professor = 'Assistant Professor', Associate_Professor = 'Associate Professor')
...
lm_DF$SR <- ifelse(test = lm_DF$SR == 0, yes = "Unsuccessful", no= "Successful")
lm_DF$SR <- as.factor(lm_DF$SR)
table(lm_DF$SR)

file_name = 'ModelData.csv'
write.csv(lm_DF,
...
set.seed(42)
lm_DF$NP <- relevel(lm_DF$NP, ref = "3")
lm_DF$DWH <- relevel(lm_DF$DWH, ref = "3")
